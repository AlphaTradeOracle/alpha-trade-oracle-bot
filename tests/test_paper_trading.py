...
class TestPaperTrading:
    ...
    async def test_open_and_scale_out_to_breakeven(self, session: AsyncSession) -> None:
        settings = Settings(
            ...
            paper_htf_breakout_enabled=False,
        )
    ...
    @pytest.mark.asyncio
    async def test_htf_opens_as_pending_without_cash_lock(self, session: AsyncSession) -> None:
        settings = Settings(
            enable_paper_trading=True,
            paper_initial_balance=2000.0,
            paper_margin_per_trade=100.0,
            paper_leverage=5.0,
            paper_fee_percent=0.0,
            paper_htf_breakout_enabled=True,
        )
        service = PaperTradingService(settings)
        result = make_result(
            direction=SignalDirection.STRONG_LONG,
